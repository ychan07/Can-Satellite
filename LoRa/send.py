    ...
    while True:
        ...
        while True:
            ...
            if char == '\x7f' or char == '\x08':
                if get_rec: # 문자열이 비어있지 않은 경우에만 지움
                    get_rec = get_rec[:-1] # 마지막 문자 제거
                    sys.stdout.write('\b \b') # 커서 뒤로 이동, 공백으로 덮어쓰기, 다시 뒤로 이동
                    sys.stdout.flush()
            else:
                get_rec += char
        ...
        if select.select([sys.stdin], [], [], 0) == ([sys.stdin], [], []):
            ...
            if c == '\x69':
                ...
                while True:
                    ...
                    # 백스페이스 처리 (ASCII DEL: \x7f, Backspace: \x08)
                    if char == '\x7f' or char == '\x08':
                        if initial_input: # 문자열이 비어있지 않은 경우에만 지움
                            initial_input = initial_input[:-1] # 마지막 문자 제거
                            sys.stdout.write('\b \b') # 커서 뒤로 이동, 공백으로 덮어쓰기, 다시 뒤로 이동
                            sys.stdout.flush()
                    else:
                        initial_input += char
